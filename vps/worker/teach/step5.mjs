// Step 5 — login → slot dashboard → click "Resubmit paper" → click "Confirm"
// in the resubmission modal → dump what comes next (file upload form).
...
// Click "Resubmit paper:" (btn-link paper-upload-modal)
...
const confirmed = await p.evaluate(() => {
  ...
  return {
    ...
    cls: (target.className?.baseVal ?? target.className ?? "").toString().slice(0, 120),
  };
});
console.log("[diag] confirm clicked:", JSON.stringify(confirmed));

await p.waitForLoadState("networkidle", { timeout: 30000 }).catch(() => {});
await new Promise((r) => setTimeout(r, 4000));
await shot(p, "after-confirm");
...
console.log(JSON.stringify({ step: 5, session: SESSION, post }, null, 2));
