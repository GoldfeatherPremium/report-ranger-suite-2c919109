...
begin
  ...
  left join lateral (
    select max(u2.freed_at) as last_freed
  ...
  where  s.is_active and a.is_active
    ...
    and  (lu.last_freed is null or lu.last_freed <= now() - interval '24 hours')
  order by (lu.last_freed is not null), lu.last_freed asc nulls first
  limit 1
  for update skip locked;
